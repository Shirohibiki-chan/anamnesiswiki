// A page drawn as a table of the pages inside it. Phase 23, step 1.
//
...
export function DatabaseTable({ node }: { node: Node }) {
  const { rows, columns, cell } = useDatabase(node);
  ...
  return (
    <div className="database-view">
      {/* Says what the grid is a grid *of*, which is the one thing a column
          of blanks can't tell you — a Location among the characters has empty
          cells for a reason, and this is where that reason is written. */}
      <div className="ui-eyebrow database-meta">
        {rows.length === 1 ? "1 page" : `${rows.length} pages`}
        {templateKey && <> · {getLabel(templateKey)}</>}
      </div>
              ...
              {rows.map((row) => (
                <tr key={row.id}>
                  <th scope="row" className="database-th-name">
                    {/* A button rather than the whole row being clickable:
                        step 3 puts editable cells in this same row, and a row
                        that navigates when you click into a cell would fight
                        them. The name is the way in, the way it is in the
                        tree. */}
...
function Cell({ value }: { value: ReturnType<ReturnType<typeof useDatabase>["cell"]> }) {
