...
- **Colours you mix are kept, and follow you everywhere.** A colour from the system picker joins a row of your own below the palette — available on any page, any block, any meter, in any project. Eight are kept, newest first, and the × on one throws it away. Only the colour you settle on is kept: moving around the picker no longer saves every shade you pass through, or write each one to disk on the way.
