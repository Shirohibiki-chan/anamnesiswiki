...
- **Jump straight to a page.** Once a grid has more pages than it draws dots for — which the pictures in the sidebar and the picker always do — the page counter under it is now a box you can type a number into, with arrows either side of it that go all the way to the front and all the way to the back. Enter takes you there, a number past the end lands on the last page, and Escape puts the box back if you change your mind. Rows of dots are unchanged: every page in one is already a single click away, first and last included.
