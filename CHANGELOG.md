...
- **Typing works the moment it opens, and `:joy:` finds what you'd expect.** The search box takes your keystrokes straight away — it didn't at first, which meant the picker came up and then swallowed everything you typed. Colons are ignored inside it, so the `:word:` habit from every chat app lands on the right emoji instead of matching nothing.
- **Searching looks through both tabs at once.** The tabs are for browsing; type anything and you get the icons *and* the emoji that match, whichever tab you happen to be on. Before, searching for an emoji from the Glyphs tab said nothing matched while the emoji sat one click away.
