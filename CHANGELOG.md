...
- **You can put an icon in a line of writing, and click it to change it.** Type `/icon` and a heart lands where you were typing; clicking it opens the picker you already know — the search box, all fifteen hundred glyphs, and the emoji tab. It stays an icon after it lands, which is the difference between this and dropping an emoji in: an emoji is a letter the moment it exists and the only way to change one is to delete it and start again. Nothing asks which icon you want first, so the sentence you were writing carries on and you fix the heart later if you want something else.
