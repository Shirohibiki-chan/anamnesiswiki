...
- **The colon key does more than it did.** It used to open the editor's built-in emoji grid; it now opens a menu with both — every glyph in the library and the same full set of emoji as before, searched together. An emoji still goes in as a plain character, exactly as it always did. A glyph goes in as an icon you can click. A bare `:` shows glyphs; the emoji appear once you've typed something to match them against.
