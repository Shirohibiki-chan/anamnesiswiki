...
- **Gradient tokens.** The user has one concrete aesthetic want and it's gradients — confirmed twice now. The sandbox's twelve slots (`--gradient-bg`, `-topbar`, `-sidebar`, `-page`, `-props`, `-modal`, `-accent`, `-sel`, `-tag`, `-title`, `-heading`, `-callout`) are the proposed shape; whichever she actually uses is the set worth building. Text gradients need their `-clip`/`-fill` pair, and the callout one layers over each callout's tint rather than replacing it. Add them to the token system up front rather than retrofitting.
- **Bundle whatever fonts she picks.** Anything from the sandbox library is OFL/Apache and can be added to `public/fonts` + an `@font-face` in `src/index.css`. Anything from the "on this PC only" group can't be, and she'll need talking through that.
