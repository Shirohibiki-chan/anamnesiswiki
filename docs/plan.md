...
**What is left of this phase**, none of it blocking:

- **A folder still has no button.** `FolderView` draws its own centred landing card and has no name row to put one on. Where it belongs there is a real question rather than an oversight.
- **The reach and the filters are not remembered** between openings, deliberately — see step 2 — and only the arrangement is. Worth revisiting if reaching for the same setting every time becomes the annoyance rather than the alternative.
- **A page's graph widened to everything and the rail's graph disagree about names.** They draw the same pages, but the pinned centre makes the layout a little larger, which can drop it under the zoom where names stop being written while the other stays above it. Two routes to one picture that do not look identical is a rough edge rather than a fault.
- **Nothing has been drawn at the scale where the layout would be slow.** The simulation runs to a stop in one go, which is a few milliseconds over 71 pages; where that stops being true is unmeasured.
