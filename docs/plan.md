...
- **Switch/close project affordance.** There's currently no in-app way to leave a loaded project and get back to the picker — closing the window is the only exit. Worth a small "Switch project" entry somewhere in the shell (top bar or a menu) once there's a natural place to put it.
