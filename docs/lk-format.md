...
| `panel` (`panelType`) | `calloutInfo` / `calloutQuote` | `info`→Info, `note`→Quote, `warning`→Info coloured amber, `error`→Info coloured red, `success`→Info coloured emerald, anything else→Info. **Changed 2026-08-28**: `warning` and `error` used to become `calloutSecret`, which was wrong — Secret is the block a publish must strip, so every imported warning was silently marked do-not-show. Phase 19.5 gave callouts a colour, which is the axis severity belongs on. Only the first paragraph child becomes the callout's content; further children flatten out as plain sibling blocks right after it (callouts here are single-inline-content, not containers). |
...
| `extension` whose `parameters.embedUrl` (or `url`) is a YouTube, YouTube Music, Spotify or SoundCloud link | `mediaEmbed` | **Since Phase 31 (2026-09-21).** LK's `block-youtube` holds `https://www.youtube.com/embed/<id>?si=…`, one of the forms `parseMediaLink` reads; the block comes in as a real player, unfetched, and asks the service for its title and still the first time the page is opened online. Lossless, uncounted. |
| `extension` (anything else) | A plain paragraph with a link | No equivalent; flagged lossy. |
...
- **`SPOTIFY_SINGLE`** → a `media` block in the sidebar (Phase 31, 2026-09-21), titled by the property's title, **when it holds a link**. One with an empty `url` — which is most of them in the real export, where every character carries an empty AMBIENCE and VIBE — is skipped, since an empty box asking for a link on every page is not what an empty LK slot was. A page that gets one has its block list written out whole (the derived blocks, then the players); every other page keeps none.
- **`TAGS` (the property, distinct from the resource's own flat `tags[]`), `SUBPAGE_INDEX`** → no equivalent field; silently skipped (shipped empty/unused in the real export).
...
| Ours | Becomes | Note |
|---|---|---|
| `calloutInfo` / `calloutQuote` | `panel` `info` / `note` | Straight inverse, **plus the severity a colour carries** (2026-08-28): an Info coloured amber, red or emerald goes out as `warning`, `error` or `success`, which is what it came in as. Any other colour is ours alone and exports as a plain `info` panel — the colour does not survive the trip. A colour on a Quote never does: LK's `note` panel carries no severity. |
| `calloutSecret` | `bodiedExtension` `block-secret` | **Not** a `panel`, and lossless in both directions as of 2026-08-28 — import no longer folds `panel` warning/error in here, so a Secret going out is a Secret somebody meant. |
...
| `mediaEmbed` (YouTube / YouTube Music) | `extension` `block-youtube` | **Phase 31.** `embedUrl` is `https://www.youtube.com/embed/<id>` (`embed/videoseries?list=<id>` for a playlist), with the `extensionType`, `text` and `layout` attrs copied from a real export. The caption follows as its own paragraph. Lossless for a page that came in with one. |
| `mediaEmbed` (Spotify / SoundCloud) | a paragraph holding the link | LK's keys for its Spotify block are not known from any export, and a guessed key is a file LK may refuse — so the link goes as a link and is counted ("1 Spotify or SoundCloud player in the writing goes across as a link"). |
| sidebar `media` block (Spotify) | `SPOTIFY_SINGLE` property | The block's title (or "Spotify") and its link. A sidebar player from another service has no LK property to be and stays here — as every sidebar block but the properties does. |
...
| `icon` inline content | an emoji goes out as text; a glyph is dropped | **Added 2026-09-01** with the inline icon. An emoji is stored as its own character, so it survives as itself. A Lucide glyph is a drawing with no character behind it and LK's own decorative icons carry no identity either (see `inlineExtension` in the import table), so there is nothing to write — and writing the name would put the literal word "sword" into her prose. The one place this file's never-silently-drop principle is deliberately not followed, because the alternative corrupts the sentence. |
