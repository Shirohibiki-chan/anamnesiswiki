...
| `mediaSingle` → `media` (a picture placed in the text) | **Nothing — dropped** | **Not yet handled, and silent about it.** No case exists for either node, so it falls to the default branch, which recurses into children the block-level switch then discards. The picture doesn't arrive broken; it doesn't arrive. Measured 2026-08-13: `Valeraverse.lk` contains **0** of these, her second account's export contains **28**. Fixing it means a case producing BlockNote's image block plus the url added to `pendingImages` so it downloads like a portrait does. `mediaSingle` also carries `attrs.width` (a percentage) and `attrs.layout`, both expressible in BlockNote's image block and both currently unread. See `docs/plan.md`. |
