...
Not a feature. Screenshots from two bot creators, sent by the user 2026-08-31, and worth keeping because **they are the target demographic and they are not her**: bot creators are usually roleplayers too, which is the overlap. Both vaults are Obsidian, so some of what follows is that tool's jank rather than a preference — the note says which.

**The two top-level schemes have almost nothing in common.** One is organised by *workflow stage*: `01 - Published`, `02 - WIP`, `03 - Concepts, drafts, ideas`, `04 - Prompt work`, `05 - Lorebooks`, `06 - Misc`. The other is organised by *canon*: `ACD`, `Marvel`, `Original Characters`, `Other Canons` — the last holding eighteen fandoms from *Bones* to *The Witcher*. **So there is no default arrangement to ship**, and a template or starter project that assumes either one is wrong for half the audience. What both do have is a `Templates` folder and a `Pictures` tree.

**Pictures sit in a parallel tree, and the character's name is welded into the filename to reconnect them.** The canon-organised vault has a top-level `Pictures` holding `ACD` and `Marvel`, and inside those a flat pile: `Anna Background.png`, `Bruce Banner.png`, `Clint Background.png`, `Eric Background.png`. **This is independent evidence for the image gallery**, from someone who has never spoken to the person who asked for it: both worked around the same missing thing, one by prefixing folders (`Damien_Pics`) and one by prefixing files. A picture belongs *to* a character and the tool gives them nowhere to say so, so they say it in the name.

**The same name appears at more than one depth, routinely.** `Irene Adler` is a top-level character and also a page inside `Jamie Moriarty`. That is normal for this kind of writing — a page about a pairing, or a variant of a character as another character writes them. **It makes name disambiguation less narrow than it looked** when the image gallery entry priced it: the `@` menu, the wikilink picker and Ctrl-K all show pages by name, and this audience genuinely has several pages sharing one.

**The numeric prefixes are a workaround we have already fixed, which nobody arriving will know.** `01 -` through `06 -` exist to force an order out of a tool that only sorts alphabetically; our tree order is manual and lives in `project.json`. **The import must not "helpfully" strip them** — they are part of the names people chose, and a folder that arrives renamed is an importer that cannot be trusted. Worth a line in Phase 20 rather than a behaviour.

**Two failures visible in the screenshots that are already rules here.** Long picture names truncate with an ellipsis and no way to read the rest (`Bruce Backgro…`, `bucee barnes f…`), which is the dead-end truncation this project already refuses. And pasted images land at the vault root as `Pasted image 20260628...`, unnamed forever — which is the argument for capturing a picture's original name at the one moment it exists, as `.names.json` does.

**One open question this raises about Phase 22 — Universes.** Universes were designed for variants of *one* world: Canon, Demonic AU, Merfolk AU. This audience keeps eighteen unrelated fandoms side by side, which is a different shape — closer to separate projects than to variants of one. Whether the universe selector serves them, or whether they want something else entirely, is not answered by the design as it stands and is worth deciding before Phase 22 is built rather than after.
