...
- ~~**A shape links to a page.**~~ Done the same day: a *Link to page* picker in the board's top-right slot writes `anamnesis://page/<id>` into the selected shape, `onLinkOpen` takes every click and sends a page link to the page and a web address to the browser, hand-typed names in the library's own link box resolve by the storyline note's rule, and `link-index.ts` records a `board` mention. What is *not* done: the library's own link popup shows the raw `anamnesis://page/…` string for a linked shape, and the only fix is inside the library.
...
**Both vaults are Obsidian, so most of what is visible in them is that tool's jank rather than anyone's preference, and jank is not a finding.** Only what changes a decision here is written down; a workaround for something we already fixed, or already decided to fix, is not evidence of anything except the thing we already knew.

**The two top-level schemes have almost nothing in common.** One is organised by *workflow stage*: `01 - Published`, `02 - WIP`, `03 - Concepts, drafts, ideas`, `04 - Prompt work`, `05 - Lorebooks`, `06 - Misc`. The other is organised by *canon*: `ACD`, `Marvel`, `Original Characters`, and an `Other Canons` holding eighteen fandoms. **So there is no default arrangement to ship** — a starter project or template that assumes either one is wrong for half the audience.

**Several pages share one name, and this was already true in her own project.** She pointed that out 2026-08-31 and it is the better example than anyone else's vault: Valeraverse holds many pages called Sampo and many called Valera. **So name disambiguation is not the narrow concern the image gallery entry priced it as** — the `@` menu, the wikilink picker and Ctrl-K all list pages by name, and the person the app is being built for already has the problem.

**The reason there are ten of a character is external, and worth understanding before designing anything around it.** Bot-hosting sites do not all support multiple greetings, so a variant of a character that would be one entry with several openings has to be published as a separate bot. She has ten Sampo bots out right now. The other vault's `Jamie Moriarty` holding a page called `Jamie Moriarty` is likely the same thing wearing a folder.

**Her own answer is tabs, and she offered it as an example rather than as a design.** A page already carries named tabs with their own content, addable, renameable and hideable (`addTab`, `renameTab`, `toggleTabHidden`), while the page's name, portrait, banner, properties and tags stay shared across all of them. One Sampo page with a tab per published variant is shared identity plus per-variant prose, built out of something that already exists. **She was explicit that this is what she would do and not what the app should assume** — the two vaults above already show people organising differently, and someone who keeps each variant as its own page is not doing it wrong.

**That mostly closes the gap this looked like opening.** `plan.md` → Phase 22 rejected base profiles with per-AU overrides as pure indirection, and that reasoning holds. Tabs sidestep it rather than contradict it: **a tab is not a patch on another page, it is a section of the same page**, so the mostly-identical case gets what overrides would have given it with no inheritance, no second file to keep in step, and no "am I editing canon or this variant" in front of every keystroke. Nothing new is needed for the way she works.

**What is genuinely left over is the other half of the audience** — people keeping each variant as its own page, which the platforms push them towards. For them the open questions are about telling ten similarly-named pages apart and about which one a picture belongs to, which are the naming and image-gallery questions above rather than anything about linking pages together.
**Their numeric folder prefixes are a workaround for alphabetical sorting, and the useful part is what the importer does with them.** Our tree order is manual and lives in `project.json`, so `01 -` through `06 -` buy nothing here — but **the importer must not strip them.** They are part of the names people chose, and a folder that arrives renamed is an importer nobody trusts. A line for Phase 20, not a behaviour.

**One open question this raises about Phase 22 — Universes.** Universes were designed for variants of *one* world: Canon, Demonic AU, Merfolk AU. This audience keeps eighteen unrelated fandoms side by side, which is a different shape — closer to separate projects than to variants of one. Whether the universe selector serves them is not answered by the design as it stands, and is worth deciding before Phase 22 is built rather than after.

---
**Relationships that write their own other half**

Wanted, 2026-08-31. Unscheduled. The clearest case of the standing line in `plan.md` → Project Overview about automating the repetitive parts.

**The problem is that a relationship is written twice or it is wrong.** Put Damien in Valera's Friends and Damien's own Friends stays empty — he shows up under her Backlinks and nowhere else. Nothing is broken, but the two pages disagree, and the half that gets forgotten is always the one nobody is looking at while writing.

**Most of the machinery is there.** `link-index.ts` already records every pointer *and why it exists*: a `Mention` carries `kind: "property"` and the field's label, which is what a Backlinks row uses to say a page arrived through Friends rather than through prose. So the app already knows Valera points at Damien through a named field. What it does not have is any notion that a field has an opposite.

**The pairing table is the actual design work, and it is small but not obvious.** Friends pairs with Friends, and so does any symmetric field. Leader pairs with Members, and *Part of* pairs with whatever a location calls its contents — those are asymmetric, so the table has direction in it, not just pairs. Some fields pair with nothing and must be left alone. `template-registry.ts` is where the `refs` fields are declared and is the natural home for it.

**Offered and switched on, never silent** — the rule the automation line carries. Writing to a page she is not looking at is the kind of helpfulness that makes an app untrustworthy, so this asks, or shows what it did somewhere she can undo it. Phase 19's undo already covers a property edit.

**Not this: turning names in prose into links.** Proposed the same day and refused by her on the spot, with the reason worth keeping — if she wanted a link she would type `[[`, and something that turns a character's name into a link while she is writing about that character would be intolerable. It was floated as a review list rather than as-you-type, and it is still refused; the objection is to the app having opinions about her prose at all. **Settled — don't propose it again.**

---
**Image gallery — a page of pictures that can be taken back out**

Asked for by the user 2026-08-31. Wanted, unscheduled. Most of the shape below was settled with her the same day, so what is marked open really is open and the rest should not be reopened.

**A tile is a picture. That is the whole definition, and it is the entire complaint about Notion's.** She checked theirs: what they call a gallery is a view of *pages* drawn as cards, where the image on a card is that page's cover. The word promises pictures and hands over documents, and putting thirty images in one means making thirty pages to hang them on. Ours holds pictures. Nothing about a tile is a page.

**Two anti-goals, read off Notion's gallery view, which she screenshotted the same day.** Both are the same failure: a thing called a gallery that is not about the pictures.

- **You cannot see the pictures.** Every card crops to the same wide rectangle whatever shape the picture is, so a portrait image — most character art, and most of hers — has a band taken out of its middle and the rest thrown away.
- **Clicking a tile opens a page.** Not the picture. Even the caption is a page title with a document icon beside it, so the label names a document too.

**Tiles are uniform, and a picture is never cropped to fit one. Settled 2026-08-31, and she was emphatic about both halves.** Uniform because the alternative is masonry — tiles keeping their own proportions and packing around one another — which she rejected outright, with a screenshot of CharSnap's image generator as the thing never to build. Never cropped because that is Notion's failure above. **The consequence is deliberate: an odd-shaped picture sits inside its tile with space around it rather than being trimmed to fill it.** Fitting the whole picture inside a fixed tile is the one arrangement that satisfies both halves.

**A gallery's pictures are the pictures on the page.** They go there the way a picture goes onto any page, and the page draws them as tiles instead of stacked down a column — there is no separate list to curate and no "add to gallery" step. Her call 2026-08-31, on a question asked badly enough that it had to be restated; this is the version she answered.
...
**Captions use the name we already keep.** A picture's file is a UUID and its name is a label in `.names.json`, so a tile has something to draw without inventing a second field. **A picture with no name shows no caption** — her call; an id, or an invented "Untitled", is worse than a blank.
...
**Taking pictures back out: select tiles, then one button — and the button says "original quality" in those words.** Her call 2026-08-31, over a download control on every tile, because one control covers one picture or forty. *Save a copy* has done the work since Phase 16: it goes through the OS save dialog, copies the original bytes and refuses to convert, and it exists because BlockNote's own Download button calls `window.open`, which is not a download in a Tauri window. None of that is visible from a button marked Download, and *just in case* is a promise about fidelity — so say it where it is read.

**A tile can make its picture the page's portrait.** `node.image` is already that slot, so showing which tile holds it and setting it from there is close to free and saves a trip to the properties panel. Wanted, 2026-08-31.

**If deleting is ever part of the selection, it goes through `asset-usage.ts`**, which knows all four places a picture can be in use and exists so that a delete button can be trusted. A picture deleted from a gallery while another page is still showing it is the failure that file was written to prevent.

**No albums inside a gallery. Settled 2026-08-31.** The reference app has them — chips with counts, an *All* and an *Unsorted*, a Create album box — and the gallery under Damien is already the grouping, so a second place to file a picture is the thing this feature exists to save her from. Also theirs and also not wanted: a Small/Medium/Large size toggle, and the whole thing being a modal with a Close button. **It looks like a page here**, which was her correction when she sent that screenshot — their app is a different app, so take the concept and leave the chrome.
...
**The qualifier goes on the ambiguous word, which is why this beats calling it just "gallery".** Notion has trained a lot of people that a gallery is cards of pages, so the bare word arrives already meaning the wrong thing; *image gallery* cannot. It also settles the objection that killed the alternative: in Notion a gallery is a view **of** a database rather than its sibling, so a plain Gallery beside a Database invites the assumption that one lives inside the other. Spelling out *image* takes the two out of that relationship.
...
**A gallery page gets that grouping for nothing.** The tree already nests and a character is already a page, so Damien's gallery is a child of Damien and there is no second set of folders to keep in order. See `CLAUDE.md` → Data on disk for why any page can hold pages, and why borrowing a folder-shaped object from Obsidian would mean importing a limitation with it.
