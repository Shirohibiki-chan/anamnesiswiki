...
**Two coping strategies, and she uses both.** Keep each variant as its own page, which is what a platform that demands separate bots pushes you towards; or hold every variant in one page and pull out what each publication needs. Her stated preference is the second, and she still ships ten separate ones — so a design that only serves one of these serves her badly half the time.

**This brushes against a settled decision and should be raised with her rather than acted on.** `plan.md` → Phase 22 explicitly rejected base profiles with per-AU overrides, on the grounds that her AUs diverge on species, appearance, history and most of the prose, so a base profile would be pure indirection. **Character variants may not be that case.** Ten Sampo bots that differ by scenario and greeting are the mostly-identical situation the rejection said would be the only one where overrides pay off. It is a different axis from universes — variants within a canon rather than versions of a world — so this is a gap the design has not covered rather than a reason to reopen what was settled.
