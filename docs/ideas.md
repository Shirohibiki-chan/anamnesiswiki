...
**The same page name appears at more than one depth, routinely.** `Irene Adler` is a top-level character and also a page inside `Jamie Moriarty` — a pairing, or a character as another character writes them. **That makes name disambiguation less narrow than the image gallery entry priced it**: the `@` menu, the wikilink picker and Ctrl-K all list pages by name, and this audience genuinely keeps several pages sharing one.
