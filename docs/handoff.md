...
- **`sandbox/theme-sandbox.html` duplicates the app's CSS and nothing enforces
  ...
  `sandbox/build-fonts.mjs` — the woff2 files are inlined as data URIs because
  a `file://` page can't load fonts any other way, and being double-clickable
  is the whole point of it.

- **The sandbox's font library is fetched from Google Fonts at *build* time, by
  a developer running `node sandbox/build-fonts.mjs`.** Not by the sandbox, not
  by the app, not at runtime, and not on her machine — everything ships as
  bytes already inside `fonts-library.css`. That's what keeps a hundred fonts
  on the right side of the Policy Boundary. If you ever find yourself adding a
  `<link>` to `fonts.googleapis.com` anywhere, that's the line, and the answer
  is to run the build script instead. Downloads cache in `sandbox/.font-cache/`
  (gitignored), so reruns are offline.

- **Every family in that library is OFL or Apache 2.0 — deliberately.** The
  sandbox also offers Windows system faces, and those are marked separately
  because Microsoft's licence doesn't let us redistribute them: picking one
  means the theme silently falls back to something else on any other machine.
  The distinction is surfaced in the UI and in the exported CSS's comments, and
  it needs to stay surfaced. Adding a family to `LIBRARY` that isn't openly
  licensed quietly breaks the promise that anything in there can be bundled.
