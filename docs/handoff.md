...
- **`sandbox/theme-sandbox.html` duplicates the app's CSS and nothing enforces
  ...
  `controls.css`, look at the sandbox in the same pass. Its `fonts.css` is
  generated by `sandbox/build-fonts.mjs` — the woff2 files are inlined as data
  URIs because a `file://` page can't load fonts any other way, and being
  double-clickable is the whole point of it.
