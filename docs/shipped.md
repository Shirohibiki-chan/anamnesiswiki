...
- **No YouTube, Spotify or map embeds**, carried from the Phase 18 scope.
  Her reason on 2026-07-31 was aesthetic rather than the offline policy,
  which she has never personally agreed with. **Lifted 2026-09-13 for
  YouTube, YouTube Music, Spotify and SoundCloud, on the condition that they
  look like they belong** — Phase 31, shipped 2026-09-21. Maps stay out.
...
**Rejected by the user the same day, and replaced — see below.** Everything in the two paragraphs above is what was built and is accurate about it; what it is missing is that none of it amounts to a title bar. Every measurement here is of the mechanism and none is of the result, which is the lesson: `drag` returning true on four elements says the window moves, and says nothing at all about four backgrounds with seams between them. The screenshot that would have shown it was skipped on the reasoning that the interesting part — the system's buttons — could not appear in one.

### An actual title bar — 2026-09-05

**The user opened the app, and the first thing she said was that it was hideous.** She was right, and the thing she asked for on 2026-08-21 was a title bar that wears the theme. What shipped that morning was the absence of one: the frame switched off, and the four things the app already drew across its top — the rail, the sidebar header, the bar above the page, and a band invented for the properties panel — made into the drag region on the reasoning that a strip of chrome did not need adding when a 48px row already existed. Four elements, four backgrounds, three seams, and window buttons tinted to a colour that only one of the four was painted in.

**What replaced it is one element.** `TitleBar.tsx`, 32px, the full width of the window, above the rail and the grid; one background, one rule under it, the app's name centred in it, and `setTitleBarOverlay` handing the system's buttons that same colour so the top of the window reads as a single strip. `.app-frame` became a flex column — the bar, then `.app-frame-body`, which is the row the frame used to be. The title is centred rather than left-aligned because Windows takes the right end of the bar and macOS the left, and the renderer cannot ask which it is running on; centre is clear on both without a platform branch.

**What came out with it.** The `-webkit-app-region` rule over four elements, the standing opt-out list every control in them needed, the `--window-controls-w` reservations on the properties panel and the bar above the page, and `.app-layout-properties-head` itself — that band existed only to be sat under. `--window-controls-w` has one reader now: the title bar's right padding.

**The rail's icons got their words.** The same session, and the same root cause — it shipped icon-only at 48px with each button's name reachable only by hovering for a tooltip, and the user could not tell what any of them were. `.left-rail-btn` is now icon over label, `--w-rail` is 84px, which is what `Templates` needs on one line, and `Switch project` wraps to two rather than ellipsising. `SettingsButton` took a `className`/`label` pair so the cog could join them without the start screen changing.

**Both moved the window's numbers.** `width` 1364 and `minWidth` 984 are 1280 and 900 plus the wider rail; `minHeight` 632 is 600 plus the title bar. `MIN_WINDOW` in the harness and the `@948`/`@1328` keys in `layout-rules.e2e.ts` moved with them.

**The scenario had to be rewritten, because it passed the whole time.** `moves-the-window-by-its-own-bar.e2e.ts` asserted that all four panels were drag regions — which was true, and which was the defect. It now checks the shape instead: one bar spanning the window, tall enough to be one, dragging, with none of those four panels still doing that job. `titleBand` in the harness grew a `strays` list for exactly that last check.

**Verified by looking at it this time**, in the built app, at both sweep widths.

**The buttons became ours the same day, and that is a decision reversed rather than a bug fixed.** The bar shipped with `titleBarOverlay`, so Windows kept drawing the real minimise, maximise and close — which kept Windows 11's snap layouts, the arrangement grid that appears on hovering the native maximise button. The price was three 46px slabs in a 32px bar, since a caption button takes a colour and a height and offers nothing else. The user asked what the slabs were; asked what keeping them bought, she did not know the feature existed and could not find it. It was a trade made on her behalf and it was not hers to want.

So `TitleBar.tsx` draws three of its own at 38px, and everything the overlay needed came out with it: `setTitleBarColors`, `readTitleBarColors`, the `window:titleBarColors` channel, the theme-store call driving them, and `--window-controls-w`. A div in the page is themed by the same stylesheet as everything else. What went in instead is smaller than what came out — `drawsWindowControls`, `minimiseWindow`, `toggleMaximiseWindow`, `watchWindowMaximised` and `requestWindowClose` on the contract, implemented on both shells.

**`requestWindowClose` is the one with a trap in it.** `closeWindow` already existed and is the *approved* close — the renderer calling it after it has flushed, skipping the handshake by design. The bar's X needed the other end: the ask that lets `onWindowCloseRequested` run first. Wiring the X to `closeWindow`, or to `window.close()`, would have taken the window without saving.

**Two verification notes worth keeping.** The overlay could never be seen — the system drew it outside the page, so a page screenshot could not include it, and it took a screen capture of a parked window to find that the bar's bottom rule was being painted over. Our buttons are elements, so an ordinary screenshot shows them and a scenario can click them. The first thing that caught with them in place was the same defect from the other side: the close button's red hover filling the bar's full height and breaking that same rule. Measured, fixed with `bottom: 1px`, measured again.
...
## Phase 28 — Getting It Back Out ✅ Shipped 2026-09-10 — phase closed
...
## Phase 30 — Home Dashboards & Quick Capture ✅ Shipped 2026-09-13 — phase closed

Scoped 2026-09-10 (#420, #421) and built 2026-09-13 in five PRs — #422 the
capture block, #423 its shortcut and palette action, #424 the style name,
#425 Recently edited and Shortcuts, and the Dashboard template with its
snippet. The section as it was planned follows, with what shipped noted
under each step; the rules that outlived the build are in `docs/handoff.md`.
...
**Shipped 2026-09-13.** A `dashboard` template in the registry — the first
whose body starts with blocks: four fixed-id blocks on the definition and a
tab of `blockRef`s and a columns row pointing at them — and `dashboard.css`,
written into the snippets folder once on first scan and switched on, aimed
at `data-template="dashboard"` and `data-style="dashboard"` both.
**Two things the build settled.** A template that declares blocks brings
them even to a page that already has some (`applyTemplate` appends the
declared ones after hers, since a pointer with nothing behind it draws as
nothing), and its override node carries a block list where every other
override still derives one from its fields. And the plan's line that
side-by-side columns were off the table was wrong by the time it was
written — the hand-built `pageColumns` row has existed since Phase 19.5 —
so the dashboard's two lists sit beside each other after all.
...
## Phase 31 — Embedded Players ✅ Shipped 2026-09-21 — phase closed
...
### 3. ~~Import, export, and the record~~ Shipped 2026-09-21

Shipped 2026-09-21 — `docs/shipped.md` § Phase 31, step 3, which closes the
phase. One line drawn on the way: the `.lk` export writes only the two
shapes known from a real export (LK's YouTube block and its Spotify
property); a Spotify or SoundCloud player in the writing goes across as a
link and is counted, rather than under a block key this app would only be
guessing.
...
### Step 3 — Import, export, and the record ✅ Shipped 2026-09-21 — phase closed

**What it delivered.** `lk-import.ts` turns an `extension` whose
`embedUrl` is one of the four services' into a `mediaEmbed` (her own
export's YouTube block holds `embed/<id>?si=…`, which `parseMediaLink`
reads) and a filled `SPOTIFY_SINGLE` into a sidebar `media` block titled
by the property — writing the page's block list out whole, derived blocks
first, since a list is absent or complete. Empty Spotify slots stay
skipped. `lk-export.ts` writes a YouTube player back as LK's `block-youtube`
with the attrs copied from the real file, a sidebar Spotify player as
`SPOTIFY_SINGLE`, and a Spotify or SoundCloud player in the writing as a
link, counted under a new lossy note. `markdown-page.ts` writes a player as
its link on its own line with the caption in italics under it, the
sidebar's under the heading the panel shows; `markdown-parse.ts` reads a
lone address that is one of the four's back into a player, caption and
all, and leaves any other lone address as a paragraph. `html-page.ts`
draws the player on the site — a YouTube still that is a link to the
video and carries the player's address for `site-script.ts` to swap in on
click, Spotify's and SoundCloud's players loaded with the page — in the
look the planner reads off the site theme's tokens
(`HtmlPageContext.playerLook`). The docs stop saying no: `handoff.md` §
Meters and `shipped.md` § Phase 18 carry the reversal with its date and
condition, `CLAUDE.md` names the four, `handoff.md` § Players holds what
binds the code, and `lk-format.md` has both tables.

**Verified.** `lk-import.test.ts` — the YouTube block comes in as a player
uncounted, a filled Spotify property becomes a titled sidebar block and an
empty one is skipped, and a page with none keeps no block list.
`lk-export.test.ts` — the YouTube player goes back as LK's block with the
caption after it, a Spotify one as a counted link, a sidebar Spotify
player as `SPOTIFY_SINGLE` and a YouTube one nowhere. `markdown-page.test.ts`
and `markdown-parse.test.ts` — the line forms both ways, a lone address
from elsewhere left alone, a link among words left a link.
`html-page.test.ts` — the still's markup exactly, Spotify's card at the
theme's address and width, the sidebar player under its heading.
`e2e/exports-a-player.e2e.ts` — the real app exports a page with a pasted
player and a sidebar one and the Markdown holds both lines. The site's
three players were rendered from the real writer and stylesheet and looked
at.

**The phase, closed.** Three steps, three PRs (#474 and the two stacked on
it), one day. What it does not do, and why: no maps (never scoped, and a
map is its own phase); no fifth service (her call, and the list is
...
## Queued Adjustments — pages inside a template named after the page ✅ Shipped 2026-09-21

The folder-preset request that sat at the top of Queued Adjustments since
2026-08-31, built as a naming rule on templates. The scoping that was in
`plan.md` is kept here in full, since it is the reasoning the code follows.

**What it does.** A template saved with sub-pages can say that each of them
is named after the page it lands in: `Damien` getting `Damien_Pics`,
`Damien_Sheets`, `Damien_Overrides`. The rule is a switch and a separator
box on the template's own page (TemplateView, shown only when the template
holds pages), with a live example. A page made from the template through the
grid or the Templates panel gets its children named; if the page is still
"Untitled" at that moment, a small prompt asks for the name first, so
nothing is ever named after an unnamed parent. Renaming the page renames the
children with it, as one press of undo; a child renamed by hand stops
following. *Add Pages From Template* on a page's right-click menu is the
second route, for a world already full of characters: the template's pages
arrive inside the page, its own writing untouched, a child whose name is
already there is skipped, and a line says how many were added — so running
it twice is safe. Saving a page whose children were named this way as a
template reads the rule back off them and stores them under their base
names, so the next page made from it is named after itself.

**How it is stored.** Two optional fields on `Node`, both in `schema.ts` with
their reasoning: `namesChildren: { separator }` on a template root, and
`nameFromParent: { base, separator }` on a page whose name was composed. On
the node rather than in a library table so the rule travels with the
template — into a shared bundle, through `cloneSubtree`, back out of Save as
Template. `services/child-naming.ts` is the pure part: composing, the
rename cascade, the skip plan, and reading the rule back. The store's
`renameNode` gained `{ byRule }` — a rename by hand clears the marker, one
by the rule keeps it, and the parent's rename folds the cascade onto itself
with `collapseSince`. A rule nested inside a rule composes from the parent's
finished name, since a template saved from a page made this way carries
`namesChildren` on the middle pages too.

### The scoping it was built from
...
**Verified** by `child-naming.test.ts` (12 cases) and
`e2e/names-the-pages-inside.e2e.ts`, which drives the whole road in the real
