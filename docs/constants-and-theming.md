...
- **`constants/theme-tokens.ts` decides what's editable** — sixteen colours in five groups, plus the twelve gradient slots. It's deliberately a subset of the token system: a theme file can set anything, but a panel offering all forty would be unnavigable.
