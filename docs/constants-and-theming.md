...
- **`storyline.ts`** — the storyline canvas's numbers: card size, zoom range, the fit's floor, and where a new note, band or scene is placed. **Deliberately separate from `graph.ts`, which it resembles** — a graph is explored and a storyline is composed, and one set of numbers serving both would mean every tuning of one silently retuning the other.
...
#### Settings is a panel at the right edge, on every section

Settings does not open as a centred dialog. It is a **full-height panel flush against the right edge** — square, no gap, no top or bottom border, a leftward shadow — with a backdrop that is fully transparent *and* `pointer-events: none`. The app beside it keeps its true colours and stays clickable, so a theme can be judged on a real page while the picker is open. Escape and the × close it; there is no click-to-close, because a panel that vanished whenever you clicked the app it exists to change would be unusable.

**Three passes, and the two rejected ones are why this is unconditional.** Reported 2026-08-30: choosing colours meant looking at the app under `--color-scrim`, a flat 50% black, so the strip that *was* visible showed the colour at half brightness and the panel could not be used for the one thing it exists for. The first fix docked the dialog aside on the four `look` sections only and slid it back to centred for the rest — reported as annoying within the week, and fairly: a window that sits somewhere different depending on which section of it you are on is a surprise every single time, and easing does not make a window that moves under you read as deliberate. Centring it everywhere instead, briefly on 2026-09-01, traded that for a box sitting on top of the page — the surface you most need to see — so it was useless in a second way. One position, at an edge, always.

- **Nothing about the panel may be keyed off which section is open.** That is the bug both earlier passes shipped, in two different properties (position, then the dim). There is no `group === "look"` branch left in `SettingsModal.tsx`, and adding one is the regression.
- **It docks at every window size**, rather than falling back to a centred dialog below some width — a fallback keyed on width is less surprising than one keyed on section, but it is still two behaviours. The width gives way instead: `--ui-modal-width: min(44rem, max(26rem, 100vw - 17rem))` keeps roughly the tree sidebar's width of app to the left of it at any size, and stops it growing past the point where the panels inside are short text on a very wide page. The rail still folds into a horizontal strip under 46rem, which is a layout change inside the panel rather than a change to where the panel is.
- **`aria-modal` is always false.** The rest of the window is genuinely reachable by mouse and keyboard; claiming otherwise is a lie to a screen reader.
- **It is pushed right with `margin-left: auto`, not a transform.** The backdrop is the same centring flex container every other dialog uses; an auto margin moves this one child to the end of it. The 2026-08-30 version used a transform because it had to animate between two positions. There is nothing to animate now.
- **The panel stays a fixed height at every size.** The old `height: auto` release under 46rem existed because a centred dialog that changes height moves its own nav by half the difference on a panel swap — click Snippets, and the rail jumps out from under the pointer. A panel pinned to both edges of the window cannot do that.

Guarded by `e2e/settings-gets-out-of-the-way.e2e.ts`, which measures the computed backdrop colour on both an appearance section and an app one, the box against the window edge, a real click landing on a tree row, that the box is byte-identical either side of crossing between the two kinds of section, and that a narrow window gives up width rather than position. None of that is decidable from the source, and a test asserting the class name would pass with the rules deleted.
