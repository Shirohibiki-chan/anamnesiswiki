...
| The rail down the left of the window — Project / Library / Templates at the top; home, back and forward, then search, switch project and settings at the foot. Each with its word under its icon | `src/components/shell/LeftRail.tsx` |
| One rail button: icon over word, dimmed when it has nothing to do. Shared by the rail and the page navigation in it | `src/components/shell/RailButton.tsx` |
