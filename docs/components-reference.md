...
| The `:` menu — the picker's grid rather than a list of rows, over glyphs from the catalogue and emoji from BlockNote's own list, plus the rule that keeps `Note:` from opening it | `src/components/blocks/IconMenu.tsx`, `src/services/editor-blocks/icon-menu-items.tsx`, `icon-trigger.ts` |
