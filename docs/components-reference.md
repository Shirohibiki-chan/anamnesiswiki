...
| The path bar above a focused tree — the top of the tree (the world, or the selected universe) → … → the node whose inside is showing. Only rendered while focused; its absence is what says you're seeing all of it. | `src/components/tree/TreePathBar.tsx` |
...
| `:` plus two letters — the single-column type-ahead over emoji and glyphs, taken with Enter or Tab | `src/services/editor-blocks/icon-menu-items.tsx`, `icon-trigger.ts` |
| Ctrl+`:` — the same picker the page title and a meter open, at the caret | `src/hooks/use-editor.ts`, `src/components/blocks/EditorIconPicker.tsx` |
...
| The infobox's own menu — its colour, the auto-adapt/fixed width pair, Full width, Align centre, Wrap left/right, Duplicate and Remove. On the strip at the bottom of the frame, since the top right corner is where the first block inside draws its own | `src/components/blocks/InfoboxMenu.tsx`, `Infobox.tsx` |
| Copy link to this block, in a block's own menu, and the mark that follows — on the block whose link was copied, and on a block a link has just landed on. A box drawn over the block rather than a style on it, since the block's own element belongs to ProseMirror | `src/components/page/BlockAnchor.tsx`, `src/services/anchor-service.ts`, `src/services/clipboard-service.ts` |
| The Quick capture box (Phase 30) — a thought typed in becomes a page under the destination shown; the type-to-filter destination picker, the code-word chip, the saved-to line, and the footnote choosing which page the destinations hang under. Deciding lives in the hook and the service. | `src/components/blocks/CaptureBlock.tsx`, `src/hooks/use-capture.ts`, `src/services/capture-service.ts` |
| The same box as a dialog — Ctrl+Shift+N or the palette footer; names whose block it is drawing, offers to add one to the home page when the world has none | `src/components/shell/QuickCaptureDialog.tsx`, `quick-capture.css` |
| Style name (Phase 30) — the right-click submenu with a name box, the normalised preview, names already in use and Clear; the same menu behind the chip on a template's view. The name lands as `data-style` on the page root, beside `data-template`. | `src/components/tree/StyleMenu.tsx`, `src/hooks/use-style-class.ts`, `src/services/style-class.ts`, the roots in `PageView.tsx`, `FolderView.tsx`, `TemplateView.tsx` |
| Recently edited and Shortcuts (Phase 30) — two more sources of the same collection block; the rows for every source come from one resolver shared with the exports, and the Recently edited count is a row in the block's menu | `src/services/collection-service.ts`, `src/constants/collection-sources.ts`, `src/components/blocks/BlockMenu.tsx` |
| The Dashboard template (Phase 30) — the definition with its four fixed-id blocks and a tab of pointers, and the snippet written once into the snippets folder | `src/services/template-registry.ts` (`dashboard`), `src/constants/dashboard-snippet.ts`, `seedDashboardSnippet` in `src/state/theme-store.ts` |
| A contents list built from the page's own headings — the block is a marker, the list is read out of the document each time it draws | `src/services/editor-blocks/table-of-contents.tsx`, `PageContents.tsx`, `src/services/toc-service.ts` |
| Linking page names written as prose — the `/` command, the preview that has to be accepted first, and the matching rules | `src/services/auto-link-service.ts`, `src/services/editor-blocks/auto-link-slash-menu.tsx`, `src/components/shell/AutoLinkDialog.tsx` |
| Image drop zone — accepts drag/drop, click-to-browse; uploads copy the file into the project's `assets/` folder and store the asset id. Named a block as well as a page since Phase 19.5: the block marked as the page's picture writes to the node, every other image block to its own record, and `usePageImage` is what says which | `src/components/properties/ImageSlot.tsx`, `src/hooks/use-page-image.ts` |
...
| A board's whiteboard — the drawing, Expand, the link and put-a-page pickers, and the page cards drawn inside the library's embeds; a card stretched to a page's worth is the page, drawn by the page's own editor and written in with it; View shows a card's page in a panel beside the board; a dropped video plays in a box; the boards inside a board are tabs along its bottom; a Layers panel on its right lists everything on it; a text box takes bold, italic and links from a toolbar over it | `src/components/page/PageBoard.tsx`, `BoardCanvas.tsx`, `BoardPageCard.tsx`, `BoardOpenPage.tsx`, `BoardViewPanel.tsx`, `BoardVideo.tsx`, `BoardSheets.tsx`, `BoardLayersPanel.tsx`, `PageReader.tsx`, `src/hooks/use-page-reader.ts`, `use-board-sheets.ts`, `board-text-metrics.ts`, `src/services/board-service.ts`, `board-videos.ts`, `board-layers.ts`, `board-text.ts` |
