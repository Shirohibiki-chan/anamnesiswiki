...
| The infobox's own menu — its colour, the auto-adapt/fixed width pair, Full width, Align centre, Duplicate and Remove. On the strip at the bottom of the frame, since the top right corner is where the first block inside draws its own | `src/components/blocks/InfoboxMenu.tsx`, `Infobox.tsx` |
