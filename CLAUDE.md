...
**Sixteen templates as of 2026-09-13 (`board`, the whiteboard spike — `docs/handoff.md` § Boards; and `dashboard`, Phase 30 — the one whose body starts with blocks; see `docs/handoff.md` § Templates that declare blocks); fourteen as of 2026-09-09; thirteen of them as of 2026-08-28** (2026-08-28 added Country, Creature, Technology, Scene and Quest, and renamed Species to Race). The rename is the one with a tail: pages written before it still say `species` on disk, and `readNodeFile` translates via `LEGACY_TEMPLATE_KEYS` on the way in rather than rewriting anyone's files. Nothing else in the app should know that word. **Race and Creature are a deliberate pair** — a Creature template for animals is what let Race stop meaning biology, which is why "Species" read as the animal one.

**`storyline` is a fourteenth template** (Phase 25, 2026-09-09). A page whose body is a canvas of scenes joined in narrative order — no tabs and no properties, since the canvas *is* the body, and a directory even when empty because its canvas lives in a file inside that directory. It is in every picker, unlike the key below, and it is still an ordinary page holding ordinary pages: its scenes are Scene pages in the tree. `docs/handoff.md` §Storylines has the rules that keep it that way.

**`universe` is a seventeenth key and not a seventeenth template** (Phase 22, 2026-09-05). It is a top-level container for one version of the world, so it has no tabs, no properties and no place in any picker — `PAGE_TEMPLATE_KEYS` is the list every picker uses and it is `TEMPLATE_KEYS` minus this one. It only ever sits at the root; `docs/handoff.md` §Storage has the rule and the marker-file rename that goes with it.
