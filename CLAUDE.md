...
**`universe` is a sixteenth key and not a sixteenth template** (Phase 22, 2026-09-05). It is a top-level container for one version of the world, so it has no tabs, no properties and no place in any picker — `PAGE_TEMPLATE_KEYS` is the list every picker uses and it is `TEMPLATE_KEYS` minus this one. It only ever sits at the root; `docs/handoff.md` §Storage has the rule and the marker-file rename that goes with it.
